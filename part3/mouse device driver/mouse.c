...
static ssize_t mouse_read(struct file *file, char *buffer,size_t count, loff_t *pos)
{
	...
	*/
	...
	mouse_dx -= dx;
	mouse_dy -= dy;
	printk("updated mouse position : (%d,%d)",mouse_dx,mouse_dy);
	...
	if(put_user((char)dy, buffer+2))
		return -EFAULT;
	for(n=3; n < count; n++)
		if(put_user(0x00, buffer+n)) Chapter 2. A simple mouse driver
			return -EFAULT;
	return count;
}
