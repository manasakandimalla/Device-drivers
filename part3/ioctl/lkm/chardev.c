...
static long ioctl_call(struct file *filp, unsigned int uiVidCmd,unsigned long arg);

static struct file_operations fops = { .unlocked_ioctl = ioctl_call,};

static long ioctl_call(struct file *filp, unsigned int uiVidCmd, unsigned long arg) 
{
  //void *argp = (void *)arg;
  copy_from_user(msg1,(char *)arg,sizeof(arg));
  ...
  copy_to_user(msg1, &msg, strlen(msg));
